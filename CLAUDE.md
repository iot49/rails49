...
* `rr-viewer` is shared by both the editor and live views — same component, `src` (image) in one, `stream` (video) in the other. Its `<img>`/`<video>` use `object-fit: contain` matched to the SVG's `preserveAspectRatio="xMidYMid meet"`, so the SVG viewBox maps 1:1 to image pixel coordinates and marker placement is identical in both modes. It **reports pointer gestures but authors nothing**: the editor was reduced to v4-supportable surfaces in #19, and #27 gave the viewer typed `rr-pointer-*` events carrying image-pixel coordinates plus a pure `ui/src/geometry.ts` (car width from DPT, span→rectangle, hit-testing) — provisioning for the editor spec, with no consumer yet, so nothing places or drags anything. See `ui/CLAUDE.md`.
