...
**SPEC § Format is now implemented.** The manifest is **v4** — cars as two-point spans with provenance, sensors per layout, multi-point calibration, `labeled_complete` per image — and `@occupancy/r49` reads and writes nothing else. **every authoring surface of the v4 editor is now built** — the calibration-point tool landed in #28, its drag in #30, the right-click context menu in #29, the tool palette, calibration gate and sensor authoring in #31, car authoring in #32, chaining with shared coupler handles in #33, reclassify in #35, and the completeness affordance in #36 — but the editor is not finished: #37 (reveal after undo) and the bugs #42–#43 are open — #41, the viewer's two independent letterboxes, is fixed, and the proposal interaction stays unresolved in `SPEC.md` § In scope. What remains furthest ahead of the code is **the detector** (nothing trains, exports, or runs YOLO yet; `detector.*` in `config.yaml` is values only). See `ui/CLAUDE.md` for the editor's state.
...
* `rr-viewer` is shared by both the editor and live views — same component, `src` (image) in one, `stream` (video) in the other. Its `<img>`/`<video>` and its SVG overlay both cover the container and fit their content into it by the same rule — `object-fit: contain` against `preserveAspectRatio="xMidYMid meet"` — over a viewBox that is the media's own pixel grid, so the two letterbox to **one box** and a marker lands on the pixel it names in either mode (#41). The authored frame (`camera.resolution`) reaches that grid through the overlay's content-group scale. It **reports pointer gestures but authors nothing** — the editor was reduced to v4-supportable surfaces in #19, and #27 gave the viewer typed `rr-pointer-*` events carrying image-pixel coordinates plus a pure `ui/src/geometry.ts` (car width from DPT, span→rectangle, hit-testing). #28 gave those events their first consumer: `rr-editor-view` places and edits **calibration points**, #30 made them draggable at one history entry per gesture, and #29 added the state-dependent right-click and the context menu that deletes through it. #31 added the tool palette, the calibration gate that disables the labeling tools while DPT is unresolved, and **sensor authoring** through those same shared paths. #32 added **car authoring** — two clicks on the visible car ends, one `image` history entry per car, drawn as a chord inside the translucent width rectangle DPT derives, which is what the calibration gate exists for — and #33 made those clicks a **chain**: every click after the first ends one car and starts the next, a rubber band shows the chain in flight, right-click ends it, a coupling renders as one shared handle that drags both cars as one entry, and undo is intercepted by the chain rather than reaching past it. #35 added **reclassify** — a context-menu submenu generated from `detector.vocabulary`, with `ui/src/vocabulary.ts` as the only place in `ui/` a class name comes from (the class a new car is created as included), and a car whose class names no vocabulary entry drawn in red with the class beside it. #36 added the **completeness affordance** — a `labeled_complete` checkbox for the image on screen, a badge per image in the thumbnail bar, one history entry per toggle, and the decision that deleting a car clears the flag (`SPEC.md` § Labeling completeness). See `ui/CLAUDE.md`.
