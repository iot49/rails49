...
def get_gauge(scale_name: str, config: dict) -> float:
    ...
    if standard_gauge is None:
        raise ValueError("Fatal Error: 'layout.standard_gauge' is missing in config.yaml.")
        
    scale_to_ratio = layout.get("scale_to_ratio")
    if not scale_to_ratio:
        raise ValueError("Fatal Error: 'layout.scale_to_ratio' is missing in config.yaml.")
        
    ratio = scale_to_ratio.get(scale_name)
    if ratio is None:
        raise ValueError(f"Fatal Error: Scale '{scale_name}' is missing in layout.scale_to_ratio in config.yaml.")
