...
For pixel data to map to the same layout location, the camera position must not change across images or during classification. **This is no longer assumed silently: drift is detected and classification refused** ([issue #12](https://github.com/iot49/rails49/issues/12), built under [map #89](https://github.com/iot49/rails49/issues/89)). Nothing records the camera pose, and nothing needs to — every image in an archive was shot from the canonical position by construction, so **the archive is the pose record** and a reference is derived from its images at load time rather than stored. `@occupancy/drift` compares the structural content of a frame against them (block-wise phase correlation, which reads geometry and discards illumination) and reports a displacement in `camera.resolution` pixels; past `layout.max_drift_px` the live view withholds every sensor as `unknown` with reason `drift` — with a visible override — and the editor warns without blocking on an image being added.
