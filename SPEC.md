...
**`camera_height_mm` is optional in the schema and required by the editor** ([issue #139](https://github.com/iot49/rails49/issues/139)). Within one layout and camera the bias largely cancels — the same DPT scales the training images and the inference frames — but **across contributors it does not**, and a ~9% spread between a baseboard-calibrated and a railhead-calibrated archive is exactly the variance [Input geometry](#input-geometry-is-dpt-normalized) exists to remove. The cost is therefore unmeasurable today and *unfixable later*, since nobody recalls where a camera stood months on, while supplying it is one tape measurement per layout. So the editor keeps one declared list of **required layout metadata** — `scale`, layout `name`, `camera_height_mm`, extensible — and the settings dialog opens **undismissable until every one is supplied**, on create and on any open that is missing one. An archive carrying them all opens straight through.
...
* **They add no version.** Settled in [issue #139](https://github.com/iot49/rails49/issues/139). The version number marks changes that make an old file **misread** or a new file **lose data**; an additive optional field is neither — an old archive loads unchanged, and a new archive is *refused* rather than misread, which is the loud failure strictness was written to produce. A bump would do the opposite of what it looks like: with `version` a literal and no compatibility shim by decision, v5 makes **every existing v4 archive fail to load**, sacrificing the backward compatibility [the frozen fixture](#migration-is-a-one-time-conversion-not-a-feature) exists to hold in order to reword a forward-compatibility refusal. Because a stale reader's raw `Unrecognized key(s)` message is useless to the model railroader who meets it, strict-key failures are reworded **once, at the archive-load boundary**, to say the archive may come from a newer build and the app should be reloaded.
